	...
	try:
		while 1:
			...
			for i in range(len(conns)):		#전체 유저를 i에 각각 저장
				conns[i].send(read)		#유저 전체에게 메세지 보내기

	except:
		print("유저 %d 님이 나가셨습니다." % (cnt))	#유저의 접속이 끝겼을때 메세지 표시
		conns.remove(conn)						#접속이 끊긴 유저를 지우기

		for i in range(len(conns)):				#나머지 전체 유저를 i에 각각 저장
			sendMessage = "유저 %d 님이 나가셨습니다." % (cnt)
